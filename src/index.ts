...
import { vttToTranscriptLines } from './utils/vtt'
...
const registerGuildCommands = async (client: Client) => {
  try {
...
const handleDiagramCommand = async (chat: ChatInputCommandInteraction) => {
...
const handleRecordCommand = async (chat: ChatInputCommandInteraction) => {
...
client.on('interactionCreate', async (interaction: Interaction) => {
  ...
  if (chat.commandName === 'diagram') return handleDiagramCommand(chat)
  if (chat.commandName === 'record') return handleRecordCommand(chat)
})
