...
import { transcribeWithWhisper, ensureWhisperAvailable } from "./whisper";
...
function extractNodes(raw: string) {
  const set = new Set(
    normalizeCSL(raw)
      .split(",")
      .map((s) => s.trim())
...
function extractRelationships(raw: string) {
...
function toKumuJSON(nodes: string[], relationships: string[]) {
  ...
  for (const c of connections) {
    if (!elementLabels.has(c.from))
      elements.push({ label: c.from }), elementLabels.add(c.from);
    if (!elementLabels.has(c.to))
      elements.push({ label: c.to }), elementLabels.add(c.to);
  }
...
export default async function audioToDiagram(attachmentUrl: string) {
  ...
  // Convert
  await convertTo16kMonoWav(audioPath, wavPath);
  ...
  if (!rawNodesResp.success)
    throw new Error(rawNodesResp.error || "LLM failed extracting nodes");
  ...
  if (!rawRelsResp.success)
    throw new Error(rawRelsResp.error || "LLM failed extracting relationships");
  ...
  return '' // @todo generate KUMU link with JSON encoded in URL and return URL
}
